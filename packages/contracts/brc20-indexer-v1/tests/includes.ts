...
export { Chain, Clarinet, Tx, types };
export type { Account };
export { contractNames };
export { noneCV, principalCV, someCV, tupleCV, uintCV };
  ...
  typeof input === "string"
    ? uintCV(
      input[input.length - 1] === "n"
        ? BigInt(input.substring(0, input.length - 1))
        : BigInt(input),
    )
    : uintCV(input);
...
export const buff = (input: string | ArrayBuffer) =>
  typeof input === "string"
    ? input.length >= 2 && input[1] === "x" ? input : `0x${input}`
    : bufferCV(input);
...
export function txPackToTuple(txPack: { [key: string]: any }) {
  const expected_struct = {
    ...
    "signature-packs": (input: any) =>
      listCV(input.map((e: any) => signPackToTupleCV(e))),
  };
