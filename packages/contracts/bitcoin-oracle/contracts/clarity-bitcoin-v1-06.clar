...
;; Returns (ok { uint16: uint, ctx: { txbuff: (buff 16384), index: uint } }) on success.
...
;; Returns (ok { uint32: uint, ctx: { txbuff: (buff 16384), index: uint } }) on success.
;; Returns ERR-OUT-OF-BOUNDS if we read past the end of txbuff
...
;; Returns (ok { uint64: uint, ctx: { txbuff: (buff 16384), index: uint } }) on success.
;; Returns ERR-OUT-OF-BOUNDS if we read past the end of txbuff
...
;; Returns (ok { varint: uint, ctx: { txbuff: (buff 16384), index: uint } }) on success
;; Returns ERR-OUT-OF-BOUNDS if we read past the end of txbuff.
...
;; Returns (ok { varslice: (buff 16384), ctx: { txbuff: (buff 16384), index: uint } }) on success, where varslice has the length of the varint prefix.
;; Returns ERR-OUT-OF-BOUNDS if we read past the end of txbuff.
...
;; Returns (ok { hashslice: (buff 32), ctx: { txbuff: (buff 16384), index: uint } }) on success, and updates the index.
;; Returns ERR-OUT-OF-BOUNDS if we read past the end of txbuff.
...
;; Returns (ok { txins: (list { ... }), remaining: uint, ctx: { txbuff: (buff 16384), index: uint } }) on success, and updates the index in ctx to point to the start of the tx outputs.
;; Returns ERR-OUT-OF-BOUNDS if we read past the end of txbuff.
;; Returns ERR-VARSLICE-TOO-LONG if we find a scriptSig that's too long to parse.
;; Returns ERR-TOO-MANY-TXINS if there are more than eight inputs to read.
...
;; Returns (ok { txouts: (list { ... }), remaining: uint, ctx: { txbuff: (buff 16384), index: uint } }) on success, and updates the index in ctx to point to the start of the tx outputs.
;; Returns ERR-OUT-OF-BOUNDS if we read past the end of txbuff.
;; Returns ERR-VARSLICE-TOO-LONG if we find a scriptPubKey that's too long to parse.
;; Returns ERR-TOO-MANY-TXOUTS if there are more than eight outputs to read.
...
;;
;; Parses a Bitcoin transaction, with up to 62 inputs and 62 outputs, with scriptSigs of up to 256 bytes each, and with scriptPubKeys up to 128 bytes.
